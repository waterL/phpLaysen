...
class Db{
	...
	// 查询
	public function row($dao , $filed = '*' , $where = ''){
	...
	// 列表
	public function page($dao , $filed = '*' , $where = '' , $page = 1 , $perpage = 10 , $order = "id desc" , $like = ''){
		$where_arr = $this->where_sql($where);
	...
	// 列表
	public function all($dao , $filed = '*' , $where = '' , $order = "id desc"){
		$where_arr = $this->where_sql($where);
		$sql = "select " . $filed . " from ".  $dao . $where_arr;
		$sql .= " order by " . $order;
		$query = mysql_query($sql);
